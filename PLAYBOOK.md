...
#### Cross-Vendor Strategic Archetypes

**14. Force Multiplier (Arm a Challenger)** — *defined here for completeness, but in practice Sunstone presents this option to Steptoe rather than committing to it*
- **Description:** An incumbent vendor holds significant federal market share for a particular capability. A challenger wants to displace them but lacks specific capability. Client provides the missing capability that lets the challenger take share. Client positioned as kingmaker, not competitor.
- **Why this matters for novel-category clients:** When a client's category is genuinely new, the incumbent by definition lacks the capability. That gap is exploitable by challengers specifically because the incumbent cannot quickly replicate it. Time-window matters.
- **Why Sunstone presents but does not select:** Sunstone is an intelligence firm, not an advocacy firm. Choosing which Goliath to back vs. attack is a Steptoe decision based on factors Sunstone cannot weigh: existing client relationships at Steptoe, political alignments, fee structures, conflict-of-interest screens, timing of other engagements. Sunstone delivers the landscape of options; Steptoe selects the play.
- **Action (Sunstone):** Map all Goliaths in the federal need's competitive landscape. For each, document their position, their advantages, their gaps, and how the client's capability could complement them. Hand Steptoe the full menu. Do not pre-select.
- **Action (Steptoe):** Choose which Goliath to back based on relationship landscape and political timing. May choose multiple. May choose none and pursue direct displacement instead.

#### The Sunstone Agnostic Principle (Critical Methodology Note)

**Sunstone never tells Steptoe who to fight.**

This is a foundational principle of the Sunstone-Steptoe partnership and applies to every engagement. The intelligence firm does not pre-select winners and losers in the Goliath landscape because:

1. **Steptoe has cross-cutting relationships.** The major federal vendors (Microsoft, AWS, Google, Oracle, IBM, Booz Allen, GDIT, Leidos, SAIC, etc.) are or have been Steptoe clients. A brief that recommends "attack Microsoft" creates an internal conflict for the firm and burns the brief's usefulness.

2. **Political and timing factors are invisible to analysis.** Steptoe knows who is actively engaged with which agency, who is positioning for which administration, which capture teams are receptive vs. hostile. Sunstone can't see those factors.

3. **The brief must be reusable.** A pre-decided brief is single-use. A landscape-and-options brief is an asset Steptoe can deploy across multiple plays, multiple meetings, multiple agencies — depending on which way they decide to go in any given situation.

4. **Role clarity preserves the partnership.** Sunstone = intelligence (what's possible). Steptoe = advocacy (what to do). Manifold = capability (the technical execution). When each plays their role, the partnership scales. When the analyst tries to also be the lobbyist, value erodes.

**Practical application:** Every Lamb Package documents the full Goliath landscape with a Manifold-with-Goliath strategy for each major player. Steptoe selects which path(s) to pursue. The package includes a vendor-agnostic white paper that works regardless of which Goliath ends up being the chosen partner.

#### Three-Axis Opportunity Scoring (Replaces Vendor-Centric RSI)

The original RSI was vendor-centric — score each vendor on whether to displace or mobilize against. This was too prescriptive. The corrected framework is opportunity-centric — score each federal need (the actual lamb) on whether it represents a viable engagement.

1. **Federal Demand Size (0-10)** — how much federal money flows through this need annually? Concentration in specific programs vs. spread across many.
2. **Capability Fit (0-10)** — how directly does the client's technology address this specific federal need? Architectural match, not just keyword overlap.
3. **Field Crowdedness (0-10)** — how many distinct Goliaths serve this need? More Goliaths = more options for Steptoe = higher value of the Sunstone brief.

**Combined Opportunity Score = Federal Demand × Capability Fit × log(Field Crowdedness + 1)**

The log function on Field Crowdedness ensures that one or two Goliaths is meaningful, but going from 5 to 10 Goliaths doesn't massively inflate score (diminishing returns on optionality).
