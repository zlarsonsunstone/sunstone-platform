...
**14. Force Multiplier (Arm a Challenger)**
- **Description:** Incumbent (Goliath B) holds significant federal market share. A challenger (Goliath A) wants to displace them but lacks specific capability. Client provides the missing capability that lets Goliath A take share from Goliath B. Client doesn't compete with either — client equips the challenger.
- **Why it matters:** Category-creating clients with structurally novel offerings often cannot compete directly (no vehicle access, no past performance, too small) and cannot displace incumbents alone. But they CAN be the differentiator that lets a major prime mount a credible challenge. Client positioned as kingmaker rather than competitor.
- **Why this matters more for novel-category clients:** When client's category is genuinely new, Goliath B (the incumbent) by definition lacks the capability. That gap is exploitable by Goliath A specifically because Goliath B can't quickly replicate it. Time-window matters.
- **Action:** Identify the incumbent (Goliath B) and the credible challenger (Goliath A). Engage Goliath A's capture team with the displacement narrative: "Here's how our capability lets you win the work currently going to [Goliath B]." Steptoe role: introduce client to Goliath A's federal business development leadership; articulate the differentiation; potentially co-position with Goliath A at agency meetings to demonstrate the combined capability stack.
- **Time horizon:** Medium (3-12 months) — challenger needs time to incorporate capability into offerings and pitches
- **Priority:** P1 if Goliath A is actively pursuing federal share; P2 otherwise
- **Example pattern (Manifold/USDA):** USDA OCFO awarded 4 cloud Pool 1 task orders in 2024 (AWS, Microsoft, Google, Oracle). Microsoft's Azure has Confidential Computing primitives baked in, giving them an advantage on secure-data workloads. AWS lacks comparable operator-supplied confidential GPU capability. Manifold's offering is the capability AWS (Goliath A) needs to displace Microsoft (Goliath B) on the next round of secure-data USDA task orders. Same play available with Oracle vs. AWS, Google vs. Microsoft, etc.
- **Critical scoring difference:** Force Multiplier targets are NOT scored on "can Manifold displace them" — they're scored on "can Manifold be USED to displace them." The displacement is done by a third party (Goliath A). The lamb is mobilized against, not engaged directly.

#### Three-Axis RSI Scoring (Replaces Two-Axis)

The original Replacement Suitability Index used two axes (Federal Footprint × Replaceability). Adding the Force Multiplier archetype requires a third axis:

1. **Federal Footprint Score (0-10)** — how much federal money does this vendor win? Concentration, vehicles, agency diversity, contract stickiness.
2. **Direct Replaceability Score (0-10)** — can the client displace this vendor directly (bid against them, beat them on capability, win the recompete)?
3. **Mobilization Score (0-10)** — can the client be USED by another vendor (Goliath A) to displace this one (Goliath B)? Does the client's capability fill a specific gap that lets a credible challenger compete?

**Combined RSI score = Federal Footprint × MAX(Direct Replaceability, Mobilization).**

The MAX function captures that a vendor with high footprint is a target whether the client displaces them directly OR arms someone else to. The pursuit archetype (3, 4, or 14) determines which path applies.

**Application logic:**
- If Direct Replaceability > Mobilization → Archetypes 3 or 4 (specialty threat or niche recompete)
- If Mobilization > Direct Replaceability → Archetype 14 (Force Multiplier)
- If both are high → multiple archetype paths possible; choose based on engagement strategy
