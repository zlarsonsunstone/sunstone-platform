/**
 * analyze-corpus-entry-background
 *
 * BACKGROUND function (15-minute timeout, fire-and-forget pattern).
 * Browser POSTs and gets immediate 202 Accepted. Function processes async.
 * Status changes propagate via Supabase realtime subscription on the row.
 *
 * Defensive try/finally guarantees the entry never stays stuck in 'analyzing'.
 * If anything panics, we mark error and stash whatever context we have.
 *
 * INPUT:  { entry_id: uuid }
 * OUTPUT: 202 Accepted (immediate). Real result lands in the database row.
 */
...
const MAX_ARTIFACT_TEXT_CHARS = 60000  // ~15K tokens, leaves room for system prompt
const MAX_USER_NOTE_CHARS = 5000

// -----------------------------------------------------------------------------
// SYSTEM PROMPT (matches synchronous version)
// -----------------------------------------------------------------------------
...
- Tier 3 artifacts may contain BOTH.

You extract:
  - CLAIMS from Tier 1, 3, 4 artifacts
  - EVIDENCE from Tier 2, 3 artifacts
...
You also maintain:
  ...
  - Open questions
  - Framing questions for the consultant

THE EDITORIAL GOAL:

The brief lives or dies on COGNITIVE DISSONANCE. Surface the gap between what the prospect believes and what the federal record shows. You set up future contradiction tests.
  ...
  "proposed_evidence": [
    {
      ...
      "evidence_category": "award_history_direct | award_history_subcontract | sam_registration | sba_certification | gsa_schedule | naics_assignment | psc_assignment | agency_relationship | teaming_partner | jv_arrangement | cage_lineage | existing_lobbyist | lobbying_disclosure | lobbying_lapse | other",
  ...
  "framing_questions": [
    {
      ...
      "expected_answer_shape": "What kind of answer helps"
    }
  ],
  "open_questions": [
...
Be specific. Be evidence-grounded. Do not hallucinate. If the artifact is truncated, acknowledge that in your reasoning fields.`
...
function truncate(s, max) {
  ...
  return s.slice(0, max) + '\n\n[... TRUNCATED for analysis. Original full text on file.]'
}

function buildArtifactContext(entry) {
  const parts = []
  ...
  if (fileMetadata) parts.push('FILE METADATA: ' + JSON.stringify(fileMetadata))
  if (userNote) parts.push('\nCONSULTANT NOTE ABOUT THIS ARTIFACT:\n' + truncate(userNote, MAX_USER_NOTE_CHARS))
...
function buildProfileContext(profile, frame) {
  const parts = []
  ...
  if (frame) {
    ...
    if (frame.human_classification) parts.push('  Consultant classification:\n' + frame.human_classification)
  }
...
export const handler = async (event) => {
  ...
  if (!ANTHROPIC_API_KEY || !SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    return { statusCode: 500, headers: corsHeaders, body: JSON.stringify({ error: 'Required env vars not configured' }) }
  }
  ...
  const entryId = req.entry_id

  // ---------------------------------------------------------------------------
  // Defensive: ensure status is updated regardless of what happens below
  // ---------------------------------------------------------------------------
  ...
  try {
    ...
    if (entryError || !entryRow) {
      throw new Error('Entry not found: ' + (entryError?.message || 'unknown'))
    }
    entry = entryRow
    ...
    if (!profile) {
      throw new Error('Strategic profile not found')
    }

    // 4. Build prompt
    const artifactContext = buildArtifactContext(entry)
    ...
    const analysis = extractJSON(claudeRaw)

    // 6. Write claims (Tier 1, 3, 4)
    ...
    if (isClaimTier && analysis.proposed_claims?.length > 0) {
      const claimsToInsert = analysis.proposed_claims.map(c => ({
        ...
        source_tier: analysis.detected_tier,
      ...
      await supabase.from('profile_claims').insert(claimsToInsert)
    }

    // 7. Write evidence (Tier 2, 3)
    ...
    if (isEvidenceTier && analysis.proposed_evidence?.length > 0) {
      ...
      await supabase.from('profile_evidence').insert(evidenceToInsert)
    }

    // 8. Update profile_understanding
    ...
    // 9. Final write to entry - SUCCESS
    await supabase
      .from('surface_research')
      .update({
        analysis: analysis,
    ...
    return {
      ...
      body: JSON.stringify({ ok: true, status: 'completed', tier_detected: analysis.detected_tier }),
    }

  } catch (err) {
    // Defensive: mark entry as error so it never stays stuck
    console.error('analyze-corpus-entry-background error:', err.message)
    ...
    return {
      statusCode: 202,  // Background functions should always return success ack
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      body: JSON.stringify({ ok: false, status: 'error', error: err.message }),
    }
  }
}
